...
import Mydefination.instructions._
...
class ID_Stage extends Module {
    val io = IO(new Bundle {
        ...
        val br = new BR_OUT()
        val rj = Output(UInt(PC_RLEN.W))
        val rk = Output(UInt(PC_RLEN.W))
        val rd = Output(UInt(PC_RLEN.W))
        val reg_rdata1 = Input(UInt(PC_WLEN.W))
        val reg_rdata2 = Input(UInt(PC_WLEN.W))
        val reg_rdata3 = Input(UInt(PC_WLEN.W))
    })
    ...
    val inst = RegInit(0.U(PC_WLEN.W))
    val pc   = RegInit(0.U(PC_WLEN.W))
    when (io.ds_allowin && io.to_ds.valid) {//握手
        inst := io.to_ds.inst
        pc   := io.to_ds.pc
    }
    ...
    io.rj := inst(9, 5)
    io.rk := inst(14, 10)
    val rj_value = Wire(UInt(PC_WLEN.W))
    val rk_value = Wire(UInt(PC_WLEN.W))
    val rd_value = Wire(UInt(PC_WLEN.W))
    rj_value := io.reg_rdata1
    rk_value := io.reg_rdata2
    rd_value := io.reg_rdata3

    val ui5  = Wire(UInt(PC_WLEN.W))
    val i12  = Wire(UInt(12.W))
    val i16  = Wire(UInt(16.W))
    val i20  = Wire(UInt(20.W))
    val si12 = Wire(UInt(PC_WLEN.W))
    val si16 = Wire(UInt(PC_WLEN.W))
    val si20 = Wire(UInt(PC_WLEN.W))
    val si26 = Wire(UInt(PC_WLEN.W))
    ui5 := inst(14, 10)
    i12 := inst(21, 10)
    i16 := inst(25, 10)
    i20 := inst(24, 5)
    ...
    val decode = ListLookup(inst, 
        List(ALU_X, BR_X, 0.U(PC_WLEN.W), 0.U(PC_WLEN.W), MEM_X, RF_X, WB_X),
        Array (
    ...
    io.to_es.dest := Mux(br_op === BR_BL, 1.U(PC_RLEN.W), io.rd)
    io.to_es.pc := pc
}